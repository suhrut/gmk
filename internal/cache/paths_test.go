...
func TestScriptPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "simple bash, build.yml at root",
			root:   "/p",
			source: "/p/build.yml",
			target: "hello",
			lang:   "bash",
			want:   filepath.Join("/p", ".gmk-cache", "code", "local", "build.yml", "hello.sh"),
		},
		{
			name:   "python, nested source",
			root:   "/p",
			source: "/p/scripts/build.yml",
			target: "compile",
			lang:   "python",
			want:   filepath.Join("/p", ".gmk-cache", "code", "local", "scripts", "build.yml", "compile.py"),
		},
		{
			...
			target: "t",
			lang:   "",
			want:   filepath.Join("/p", ".gmk-cache", "code", "local", "build.yml", "t.sh"),
		},
	}
