...
// scope walker, and expression evaluator together against real build.yml
// files in the examples/ tree.
//
// The contract these tests enforce:
//
//  1. Every build.yml under examples/ must parse (load.Load returns nil err).
...
//
// To add a new example: drop a new build.yml under
// examples/<category>/<feature>/build.yml. It will be picked up
...
// build.yml found at any depth. The tree intentionally has multiple levels
// (examples/<category>/<feature>/build.yml) and may grow deeper as
...
func discoverExamples(t *testing.T, root string) []string {
	...
	err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		// Only entry-point files are named build.yml. Included library
	...
	if len(found) == 0 {
		t.Fatal("no build.yml files found under examples/")
	}
	return found
}

// TestExamplesLoad asserts every build.yml in the tree parses without error.
